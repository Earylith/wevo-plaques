...
import { Accommodation, OfferType, ContactInfo, Recommendation, PointOfInterest } from "@/lib/types/accommodation";
...
interface Props {
  ...
  onSubmit: (data: Omit<Accommodation, "id" | "createdAt" | "updatedAt">) => Promise<void>;
  isLoading?: boolean;
}

export default function AdminAccommodationForm({ initialData, onSubmit, isLoading }: Props) {
  const [formData, setFormData] = useState<Partial<Accommodation>>(
    initialData || {
      ...
      property: { name: "", type: "", city: "", address: "", welcomeMessage: "" },
      wifi: { ssid: "", password: "" },
      practicalInfo: { checkin: "", checkout: "" },
      rules: [""],
      contacts: [],
      recommendations: [],
      pointsOfInterest: [],
  ...
  const [qrUrl, setQrUrl] = useState(initialData?.slug ? `${window.location.origin}/h/${initialData.slug}` : "");

  const handleChange = (section: keyof Accommodation, field: string, value: any) => {
    if (section === "slug" || section === "isActive" || section === "offerType") {
      setFormData((prev) => ({ ...prev, [section]: value }));
      if (section === "slug") {
        setQrUrl(`${window.location.origin}/h/${value}`);
      }
    } else if (section === "comfortOptions") {
      setFormData((prev) => ({
        ...prev,
        comfortOptions: {
          ...prev.comfortOptions,
  ...
  const handleComplexArrayChange = (section: "contacts" | "recommendations" | "pointsOfInterest", index: number, field: string, value: any) => {
  ...
  const addArrayItem = (section: string) => {
    ...
    } else if (section === "pointsOfInterest") {
      setFormData((prev) => ({ ...prev, pointsOfInterest: [...(prev.pointsOfInterest || []), { title: "", description: "" }] }));
    } else if (section === "faq") {
      setFormData((prev) => ({
        ...prev,
  ...
  const generateSlug = () => {
    if (formData.property?.name) {
      const newSlug = slugify(formData.property.name);
      setFormData((prev) => ({ ...prev, slug: newSlug }));
      setQrUrl(`${window.location.origin}/h/${newSlug}`);
    }
  };
  ...
  const downloadQR = () => {
    ...
    img.onload = () => {
      ...
      downloadLink.download = `QR_${formData.slug || "logement"}.png`;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit(formData as Omit<Accommodation, "id" | "createdAt" | "updatedAt">);
  };

  const isComfort = formData.offerType === "comfort";

  return (
    <form onSubmit={handleSubmit} className="space-y-8 pb-24">
      {/* Configuration globale */}
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-[#EDD9A3]/40">
        <h2 className="text-xl font-bold text-[#2A2016] mb-4">Configuration globale</h2>
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-[#6B5D4E] mb-1">Offre</label>
            <select
            ...
            >
              ...
              <option value="comfort">Confort (Premium)</option>
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-[#6B5D4E] mb-1">Statut</label>
            <div className="flex items-center h-10">
              <label className="flex items-center cursor-pointer">
                ...
                />
                <div className="relative w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#5A7A4E]"></div>
                <span className="ml-3 text-sm font-medium text-[#2A2016]">
                  {formData.isActive ? "En ligne" : "Désactivé"}
                </span>
              </label>
            </div>
          </div>
      ...
      {/* Propriétaire */}
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-[#EDD9A3]/40">
        <h2 className="text-xl font-bold text-[#2A2016] mb-4">Propriétaire</h2>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-[#EDD9A3]/40">
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[#6B5D4E] mb-1">Slug de l&apos;URL</label>
            ...
            <div key={index} className="flex flex-wrap sm:flex-nowrap gap-2 bg-[#FBF5EC] p-3 rounded-xl border border-[#EDD9A3]/50">
              <div className="w-full grid sm:grid-cols-2 gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-lg border border-[#EDD9A3] text-sm"
                />
                <input
                  type="text"
                  placeholder="Nom"
                ...
                <input
                  type="text"
                  placeholder="WhatsApp (optionnel, ex: +33612345678)"
              ...
              <div className="flex justify-between items-start">
                <div className="grid sm:grid-cols-2 gap-3 w-full mr-4">
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-[#EDD9A3] text-sm"
                  />
                  <input
                    type="text"
                  ...
                  <textarea
                    ...
                    className="w-full sm:col-span-2 px-3 py-2 rounded-lg border border-[#EDD9A3] text-sm"
                    rows={2}
                  />
                  {isComfort && (
                    <div className="w-full sm:col-span-2">
                      <label className="block text-xs font-medium text-[#6B5D4E] mb-1">Image d'illustration (Offre Confort)</label>
                      <div className="flex gap-2 items-center">
                        {rec.imageUrl && (
                        ...
                        <input
                          ...
                          className="w-full px-2 py-1 rounded-lg border border-[#EDD9A3] text-sm file:mr-2 file:py-1 file:px-2 file:rounded-full file:border-0 file:text-[10px] file:font-semibold file:bg-[#C4714A]/10 file:text-[#C4714A]"
                        />
                      </div>
                    </div>
                  )}
      ...
      {/* QR Code Section */}
      {formData.slug && (
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-[#EDD9A3]/40 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-[#2A2016] mb-2">QR Code</h2>
            <p className="text-sm text-[#6B5D4E] mb-4">
              Pointe vers : <a href={qrUrl} target="_blank" rel="noreferrer" className="text-[#C4714A] hover:underline">{qrUrl}</a>
            </p>
            <button
              type="button"
              onClick={downloadQR}
              className="px-5 py-2.5 rounded-xl bg-[#2B5F75] text-white font-semibold hover:bg-[#1A3F52] transition-colors"
            >
              Télécharger le QR Code PNG
            </button>
          </div>
          <div className="bg-white p-2 border border-gray-200 rounded-xl">
            <QRCodeSVG id="qr-code" value={qrUrl} size={120} level={"H"} includeMargin={true} />
          </div>
        </div>
      )}

      {/* Bouton de sauvegarde */}
      <div className="fixed bottom-0 left-64 right-0 p-4 bg-white/80 backdrop-blur-md border-t border-gray-200 flex justify-end z-50">
        <button
          type="submit"
          disabled={isLoading}
        ...
        >
          {isLoading ? "Enregistrement..." : "Enregistrer le logement"}
        </button>
      </div>
    </form>
  );
